...
if [ "X$(uname)" = "XLinux" ] ; then
    ...
    (sleep 5 && chmod +x ./var/upgrade/{*.sh,src/init/*.sh} && ./var/upgrade/src/init/pkg_installer.sh && find ./var/upgrade/ -mindepth 1 -not -name upgrade_result -delete) >/dev/null 2>&1 &
fi
